...
var BBOX = arg.BBOX.split(',').map(function(elem){ // bottom left corner coords and top right corner coords of the image
           return Number(elem) })
...
var addCesty = arg.LAYERS.incudes('cesty');
...
map.fromString(schema, function(err, map) { // we use method "fromString" => we need to use the xml schema inside variable schema
  ...
  map.render(im, function(err, im) { // render the map into mapnik image stored in variable "im"
    ...
    if (err) {
        console.log('Map render Error: ' + err.message) // print an error if it occures
    }
